...
//Check for digits that have only one possible position (mark) in a set
...
bool NineSet::checkSingleRowCol(int &boxRowReturn, int &boxColReturn, int digit)
{
  ...
  {
    if (cells[cellsIndex]->isDigitMarked(digit))
    {
...
bool NineSet::checkLimitedMarks()
{
  if (solved)
    return false;
  return limitedMarksRecursionStart(9 - solvedDigits);
}

bool NineSet::limitedMarksRecursionStart(int finalDepth)
{
  bool ret = false;
  for (int cell = 0; cell < 9; ++cell)
  {
    ...
    if (!cells[cell]->isSolved())
    {
      for (int digit = 0; digit < 9; ++digit)
      {
        boolArray[digit] = cells[cell]->isDigitMarked(digit);
      }
...
void NineSet::updateChanges(bool updateMarks)
{
  for (auto &cell: cells)
  {
...
void NineSet::removeDigit(int digit)
{
  digits[digit] = false;
  solvedDigits--;
  solved = false;
...
bool NineSet::isDigitSolved(int digit)
{
  return digits[digit];
}
